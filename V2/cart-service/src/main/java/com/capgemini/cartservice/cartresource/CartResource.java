...
public class CartResource {
	...
	public ResponseEntity<Cart> updateCart(@PathVariable int cartid, @RequestBody Cart cart) {
		...
		/*
		 * Cart cartOne = service.getcartById(cartid); List<Items> items = new
		 * ArrayList<Items>(); if(cartOne.getItems()!=null) {
		 * items.addAll(cartOne.getItems()); } items.addAll(cart.getItems());
		 * cartOne.setItems(items);
		 */
		updatedCart = service.addToCart(cart);
